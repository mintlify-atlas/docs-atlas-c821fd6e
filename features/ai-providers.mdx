---
title: "AI Providers"
description: "Claude and OpenAI integration for handoff note generation and image analysis"
icon: "robot"
---
...
While Claude is currently the only provider, the architecture supports adding more:
...
app.post("/api/analyze-image/openai", async (req, res) => {
  // OpenAI vision implementation
});
```

<Tip>
The API uses a provider parameter in the URL path (e.g., `/claude`, `/openai`) to support multiple AI providers with the same interface.
</Tip>
